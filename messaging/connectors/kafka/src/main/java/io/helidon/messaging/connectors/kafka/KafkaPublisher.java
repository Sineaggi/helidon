                        ...
                        if (currentNoAck < limitNoAck) {
                            if (backPressureBuffer.isEmpty()) {
                                try {
                                    ...
                                    LOGGER.log(Level.DEBUG, () -> String.format("%s It was requested to stop polling from channel", topics));
                                }
                            } else {
                                ...
                                if (eventsToEmit > 0) {
                                    LOGGER.log(Level.DEBUG, () -> String.format("%s %s messages to emit. %s in buffer and %s requested",
    ...
    public void stop() {
        ...
        if (kafkaConsumer != null) {
            ...
            try {
                ...
                LOGGER.log(Level.DEBUG, () -> String.format("%s Buffered events that were not processed %s", topics, backPressureBuffer));
                emitter.complete();
            } catch (RuntimeException e) {
                emitter.fail(e);
            } finally {
